...
(async()=>{
 ...
 if(!await page.locator('#memory-compatibility').isChecked())throw Error('Memory compatibility must default on after upgrade');
 await page.locator('#memory-compatibility').uncheck();
 ...
 if(!await page.evaluate(()=>window.calls.some(c=>c.op==='client_start'&&c.args.memory_compatibility===false)))throw Error('Opt-out missing from native launch');
 ...
 await page.locator('[data-action="client_start"]').click();await page.waitForTimeout(50);
 if(!await page.evaluate(()=>window.calls.some(c=>c.op==='client_start'&&c.args.memory_compatibility===true)))throw Error('Opt-in missing from native launch');
