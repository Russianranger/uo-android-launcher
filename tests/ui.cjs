...
(async()=>{
 ...
 if(!await page.evaluate(()=>window.calls.some(c=>c.op==='client_start'&&c.args.client_memory_compatibility===false&&c.args.managed_diagnostics===false)))throw Error('Opt-out missing from native launch');
 ...
 await page.locator('[data-action="client_start"]').click();await page.waitForTimeout(50);
 if(!await page.evaluate(()=>window.calls.some(c=>c.op==='client_start'&&c.args.client_memory_compatibility===true&&c.args.managed_diagnostics===true)))throw Error('Opt-in missing from native launch');
