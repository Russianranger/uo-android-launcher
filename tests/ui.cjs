...
(async()=>{
 ...
 if(!await page.evaluate(()=>window.calls.some(c=>c.op==='client_start'&&c.args.runtime_backend==='fex-arm64ec-1'&&c.args.managed_diagnostics===false&&c.args.render_trace===false&&c.args.sdl_graphics_fixes===false&&c.args.audio_driver==='wasapi'&&c.args.proot_acceleration===true&&c.args.music_cache===true)))throw Error('Default options missing from native launch');
 ...
 await page.locator('[data-action="client_start"]').click();await page.waitForTimeout(50);
 if(!await page.evaluate(()=>window.calls.some(c=>c.op==='client_start'&&c.args.runtime_backend==='fex-arm64ec-1'&&c.args.managed_diagnostics===true&&c.args.render_trace===true&&c.args.sdl_graphics_fixes===false&&c.args.audio_driver==='directsound'&&c.args.proot_acceleration===false&&c.args.music_cache===false)))throw Error('Selected options missing from native launch');
