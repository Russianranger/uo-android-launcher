...
or ARM64/Box64/Turnip. Also model TazUO's replacement filter: the original
FNA-only failure must not be treated as a reproduction of TazUO's device crash.
"""
import os
...
try:
    for name,policy,expected,marker,args in (
        ...
        with (runner.LOGS/(name+'.log')).open('wb') as output:
            result=subprocess.run(['timeout','90',str(wine),str(root/'app/PaintProbe.exe')]+args,
    ...
    print('Verified: standalone FNA can reenter Draw, but TazUO-style filter replacement prevents this with either hint value',flush=True)
finally:
