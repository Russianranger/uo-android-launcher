...
function options(mode='client'){return{runtime_backend:'fex-arm64ec-1',pacing_revision:1,mode,renderer:$('renderer').value,resolution:$('resolution').value,presentation_mode:$('presentation').value,display_fps:Number($('fps').value),audio:$('audio').checked,music_cache:$('music-cache').checked,frame_budget:$('frame-budget').checked,audio_driver:$('audio-driver').value,proot_acceleration:$('client-acceleration').checked,gump_space:$('gump-space').checked,managed_diagnostics:$('managed-diagnostics').checked,render_trace:$('render-trace').checked,sdl_graphics_fixes:$('sdl-graphics-fixes').checked};}
try{const v=JSON.parse(localStorage.getItem('launch')||'{}');if(['wasapi','directsound'].includes(v.audio_driver))$('audio-driver').value=v.audio_driver;$('client-acceleration').checked=v.proot_acceleration!==false;$('music-cache').checked=v.music_cache!==false;$('frame-budget').checked=v.frame_budget!==false;}catch(_){}
function saveOptions(){localStorage.setItem('launch',JSON.stringify(options()));}
try{const v=JSON.parse(localStorage.getItem('launch')||'{}');for(const [key,id]of[['renderer','renderer'],['resolution','resolution'],['presentation_mode','presentation'],['display_fps','fps']])if(v[key]&&(key!=='renderer'||v.runtime_backend==='fex-arm64ec-1')&&(key!=='display_fps'||v.pacing_revision===1))$(id).value=v[key];if(typeof v.audio==='boolean')$('audio').checked=v.audio;}catch(_){}
...
for(const id of['renderer','presentation','fps','audio','audio-driver','client-acceleration','frame-budget','music-cache','sdl-graphics-fixes','render-trace','managed-diagnostics'])$(id).addEventListener('change',saveOptions);
...
async function refresh(){
    ...
    try{
        ...
        const music=clientState.launch?.music_cache;
        $('music-cache-status').textContent=!music?'':music.active?'Last launch: music folder cache active.':music.requested?'Last launch: music cache unavailable for this client build.':'Last launch: original music loader active.';
