...
function options(mode='client'){return{runtime_backend:'fex-arm64ec-1',pacing_revision:1,mode,renderer:$('renderer').value,resolution:$('resolution').value,presentation_mode:$('presentation').value,display_fps:Number($('fps').value),audio:$('audio').checked,audio_driver:$('audio-driver').value,proot_acceleration:$('client-acceleration').checked,gump_space:$('gump-space').checked,managed_diagnostics:$('managed-diagnostics').checked,render_trace:$('render-trace').checked,sdl_graphics_fixes:$('sdl-graphics-fixes').checked};}
try{const v=JSON.parse(localStorage.getItem('launch')||'{}');if(['wasapi','directsound'].includes(v.audio_driver))$('audio-driver').value=v.audio_driver;$('client-acceleration').checked=v.proot_acceleration!==false;}catch(_){}
...
for(const id of['renderer','presentation','fps','audio','audio-driver','client-acceleration','sdl-graphics-fixes','render-trace','managed-diagnostics'])$(id).addEventListener('change',saveOptions);
